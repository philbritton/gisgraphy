...
public class OpenStreetMapPoiFileRetriever extends AbstractFileRetriever {
	...
	return importerConfig.getOpenStreetMapPoisDir();
    }
	...
	for (int i = 0; i < filesToImport.length; i++) {
	    logger.info("the files " + filesToImport[i].getName()
		    + " will be imported for openstreetMap Pois");
	}
    }
    ...
    List<String> getFilesToDownload() {
    	return importerConfig.getOpenStreetMapPoisDownloadFilesListFromOption();
    }
