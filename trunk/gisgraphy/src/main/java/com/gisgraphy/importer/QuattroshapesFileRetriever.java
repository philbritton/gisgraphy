 ...
 */
public class QuattroshapesFileRetriever extends AbstractFileRetriever {
	...
	return importerConfig.getQuattroshapesDir();
    }
	...
	for (int i = 0; i < filesToImport.length; i++) {
	    logger.info("the files " + filesToImport[i].getName()
		    + " will be imported for quattroshapes");
	}
    }
    ...
    List<String> getFilesToDownload() {
    	return importerConfig.getQuattroshapesFilesDownloadFilesListFromOption();
    }
